            ...
            >
              {recommendedMovies.slice(0, 10).map((movie, index) => (
                <div
                  ...
                  className="flex-none w-[200px] bg-white rounded-lg overflow-hidden shadow-md hover:shadow-xl cursor-pointer transform hover:scale-[1.02] transition-all duration-200 snap-start"
                >
                  {/* Badge */}
                  {index < 3 && (
        ...
        <div className="container">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {allMovies.filter(m => ["5", "6", "9", "10", "7", "8", "4", "11"].includes(m.id)).slice(0, 8).map((movie) => (
              ...
              >
                {/* Event Icon */}
                <div className="flex-none w-16 h-16 bg-gradient-to-br from-[#8B5E3C] to-[#6D4C3B] rounded-lg flex items-center justify-center text-white font-bold text-xl shadow-md">
                  {movie.title.charAt(0)}
                </div>

                {/* Event Info */}
                <div className="flex-1 min-w-0">
                  <h3 className="font-semibold text-sm text-[#3E2723] truncate mb-1">
                    {movie.title}
                  </h3>
          ...
          >
            {recommendedMovies.slice(0, 8).map((movie) => (
              ...
              >
                ...
                />
                <div className="flex-1 min-w-0">
                  <h3 className="font-semibold text-sm text-[#3E2723] truncate">
                    {movie.title}
                  </h3>
                  <p className="text-xs text-gray-600">{movie.genre[0]}</p>
                  <p className="text-xs font-semibold text-[#8B5E3C] mt-1">
                    {movie.theaters && movie.theaters[0]?.showTimes[0] 
                      ? `₹ ${movie.theaters[0].showTimes[0].price}.00` 
                      : 'Free'}
                  </p>
