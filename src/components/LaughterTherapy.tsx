...
// ✅ FIX: correct interface name
interface LaughterTherapyProps {
  movie: Movie;         
  inCarousel?: boolean;
}

const LaughterTherapy = ({ movie, inCarousel = false }: LaughterTherapyProps) => {
  const { toast } = useToast();
  const [liked, setLiked] = useState(false);

  useEffect(() => {
    const savesFavrites: string[] = JSON.parse(localStorage.getItem("favorites") || "[]");
    if(savesFavrites.includes(String(movie.id))){
        setLiked(true);
    }
  },[movie.id])
  ...
  return (
     <div className="flex-shrink-0 w-[230px] rounded-xl overflow-hidden shadow-sm border border-[#F9F8F6]">
      ...
      <div style={{ backgroundColor: "transparent", padding: "12px" }}>
        ...
        <p className="text-sm text-white/80">{movie.language}</p>
      </div>
    </div>
  );
}; 
